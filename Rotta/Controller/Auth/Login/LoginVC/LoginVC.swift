...
class LoginVC: UIViewController {
    ...
    lazy var loginWithAppleButton: ASAuthorizationAppleIDButton = {
        ...
        button.addTarget(self, action: #selector(loginWithAppleButtonTapped), for: .touchUpInside)
    ...
    lazy var actionButtonsStackView: UIStackView = {
        var stack = UIStackView(arrangedSubviews: [
            loginButton, createAccountButton, loginWithAppleButton
        ])
        stack.translatesAutoresizingMaskIntoConstraints = false
        stack.axis = .vertical
        stack.spacing = 16
    ...
    @objc func didTapLoginButton() {
        guard let email = emailComponent.textField.text, !email.isEmpty,
            ...
            emailAndPasswordErrorLabel.text = "Email e senha não podem estar vazios."
        ...
        Task {
            do {
                if let _ = try await UserService.shared.loginUser(email: email, password: password) {
                    ...
                    emailAndPasswordErrorLabel.text = "O email e a senha que você inseriu não correspondem aos nossos registros."
                    emailAndPasswordErrorLabel.isHidden = false
                }
            } catch {
                emailAndPasswordErrorLabel.text = "Ocorreu um erro durante o login. Por favor, tente novamente."
                emailAndPasswordErrorLabel.isHidden = false
            }
        }
    }
