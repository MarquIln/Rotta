...
class DriverPodium: UIView {
    ...
    private let seeAllButton: UIButton = {
        ...
        button.setImage(
            UIImage(systemName: "chevron.right.circle.fill"),
            for: .normal
        )
    ...
    private lazy var driverStack: UIStackView = {
        let stack = UIStackView(arrangedSubviews: [
            secondPlaceView, firstPlaceView, thirdPlaceView,
        ])
    ...
    override func layoutSubviews() {
        ...
        guard
            containerStack.bounds.width > 0 && containerStack.bounds.height > 0
        else { return }

        let hasGradientLayer =
            containerStack.layer.sublayers?.contains { $0 is CAGradientLayer }
            ?? false
    ...
    func update(with drivers: [DriverModel]) {
        ...
        if sorted.count > 0 {
            firstPlaceView.configure(with: sorted[0], rank: 1)
        }
        if sorted.count > 1 {
            secondPlaceView.configure(with: sorted[1], rank: 2)
        }
        if sorted.count > 2 {
            thirdPlaceView.configure(with: sorted[2], rank: 3)
        }
    }
